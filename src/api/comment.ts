...
import type { ApiCommentResponse, ApiPostCommentResponse, PostCommentFormData } from "../types/types";
...
export const addCommentToCatPost = async (commentFormData:PostCommentFormData, catPostId:string) =>{
    ...
    try {
        ...
        const res = await axios.post<ApiPostCommentResponse>(`${URL}/catpost/${catPostId}/comments`, commentFormData, { headers } );
        return res.data;
        ...
        if (axios.isAxiosError(error)) {
            ...
            throw new Error(error.response?.data?.message || "Error desconocido al crear comentario.");
        } else {
            console.error("Error inesperado:", error);
            throw new Error("Ocurrió un error inesperado.");
        }
